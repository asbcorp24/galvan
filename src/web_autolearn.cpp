...
<section class="card">
    <h2><span class="dot"></span> Калибровка ванн (Auto Learn)</h2>

    <p style="color:var(--muted); font-size:0.9rem;">
        Контроллер автоматически проедет по линии, определяя ванны по RFID-меткам.
    </p>
    ...
    <div id="stateBox" 
        ...
        <div style="font-size:1rem; font-weight:600;">Состояние: <span id="stState">—</span></div>
        <div style="margin-top:4px; font-size:0.85rem; color:var(--muted);">
            Обнаружено ванн: <span id="stCount">0</span>
        </div>
        ...
        <div style="margin-top:10px;">
            ...
            <div style="width:100%; height:10px; background:#111; border-radius:6px;">
                <div id="stProgress" style="height:10px; width:0%; background:var(--accent); border-radius:6px;"></div>
            </div>
        </div>
    </div>

    <h3 style="margin-bottom:6px;">Последняя обнаруженная ванна</h3>
    <div id="lastFound" class="bath-card" 
         style="padding:10px; background:rgba(59,167,255,0.10); border:1px solid var(--border); border-radius:10px;">
         Пока нет данных
    </div>
    ...
    fetch("/baths_autolearn", {method:"POST"})
        ...
        .then(t => {
            document.getElementById("stState").textContent = 
                "Команда отправлена (ожидание начала...)";
        });
};
...
function renderFoundList(list){
    ...
    list.forEach(b => {
        ...
        el.innerHTML = `
            ...
            <div class="bath-line"><span class="bath-label">Старт:</span> <span class="bath-value">${b.isStart}</span></div>
            <div class="bath-line"><span class="bath-label">Конец:</span> <span class="bath-value">${b.isEnd}</span></div>
        `;
        div.appendChild(el);
    });
}
